...
while [ $i -le $n ]
do
   echo -en '\t\t{"{#CPU_PROCESS_NAME}":"'`ps -eo pid,%cpu,comm | sort -nr -k 2 | awk -v m="$percent"  '{if($2 >m){print $0}}' | awk '{a[$NF]+=$2}END{for(k in a){print a[k],k}}'| awk -v n1="${i}" '{if(NR==n1){print $2}}' `'"}' 
    if [ $i -lt $n ];then
        echo ','
    fi
    let i++
